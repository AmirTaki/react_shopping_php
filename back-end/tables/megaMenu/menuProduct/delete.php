    ...
    switch($method){
        case "DELETE";
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                $read =  readTable ($DBName, "SELECT * FROM menuseries WHERE id = ?", $single = true, $execute = [$path[7]]);
                if($read){
                
                    // operations users in dataBase
                    $response = operationsDatabase($DBName, "DELETE FROM menuseries WHERE id = ?", $execute = [$path[7]]);
