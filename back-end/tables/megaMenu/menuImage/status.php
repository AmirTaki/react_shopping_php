       ...
       switch($method){
        case "GET":
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                $user = readTable ($DBName, 'SELECT * FROM menuimage WHERE id = ?', $single = true, $execute = [$path[7]]);

                if($user){
                
                    // operations status users in data base
                    $status =  ($user->status == 10) ? 0 : 10;
                    operationsDatabase($DBName, "UPDATE menuimage SET status = ? WHERE id = ? ", [$status, $path[7]]);
