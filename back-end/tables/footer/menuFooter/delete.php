    ...
    switch($method){
        case "DELETE";
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                $read =  readTable ($DBName, "SELECT * FROM menufooter WHERE id = ?", $single = true, $execute = [$path[7]]);
                if($read){
                
                    // operations users in dataBase
                    $response = operationsDatabase($DBName, "DELETE FROM menufooter WHERE id = ?", $execute = [$path[7]]);
            
                    // reading all database
                    $users = readTable ($DBName, "SELECT * from menufooter", $single = false, $execute = null);
