        ...
        case "POST" :
            if(
                ...
                if($checkImage){
                    ...
                    // upload image
                    $path =  uploadImageMultipart('image', "/back-end/tables/session/scrollSlider/uploadImage/");
                    if($path){

                        // create to database 
                        $response =  operationsDatabase($DBName, "INSERT INTO scrollslider SET title = ?, body = ?, price = ?, image = ?, created_at = NOW()", $execute = [$_POST['title'], $_POST['body'], $_POST['price'], $path]);
