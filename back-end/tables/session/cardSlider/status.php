       ...
       switch($method){
        case "GET":
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                $user = readTable ($DBName, 'SELECT * FROM cardslider WHERE id = ?', $single = true, $execute = [$path[7]]);

                if($user){
                
                    // operations status users in data base
                    $status =  ($user->status == 10) ? 0 : 10;
                    operationsDatabase($DBName, "UPDATE cardslider SET status = ? WHERE id = ? ", [$status, $path[7]]);
                
                    // reading all database
                    $users = readTable ($DBName, "SELECT * from cardslider", $single = false, $execute = null);
