        ...
        case "POST" :
            if(
                ...
                if($checkImage){
                   ...
                   // upload image
                    $path =  uploadImageMultipart('image', "/back-end/tables/session/cardSlider/uploadImage/");
                    if($path){

                        // create to database 
                        $ersponse =  operationsDatabase($DBName, "INSERT INTO cardslider SET title = ?, image = ?, created_at = NOW()", $execute = [$_POST['title'], $path]);
