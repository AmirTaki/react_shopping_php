        ...
        case "POST" :
            if(
                ...
                if($checkImage){
                   ...
                   // upload image
                    $path =  uploadImageMultipart('image', "/back-end/tables/session/circleSlider/uploadImage/");
                    if($path){

                        // create to database 
                        $ersponse =  operationsDatabase($DBName, "INSERT INTO circleSlider SET title = ?, image = ?, created_at = NOW()", $execute = [$_POST['title'], $path]);
                ...
                echo json_encode(["message" => "title & image  is requierd !!"]);
                exit();
            }
