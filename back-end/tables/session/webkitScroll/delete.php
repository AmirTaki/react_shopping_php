        ...
        case "DELETE";
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                if($read){
                    ...
                    $response = operationsDatabase($DBName, "DELETE FROM webkitScroll WHERE id = ?", $execute = [$path[7]]);
            
                    // reading all database
                    $users = readTable ($DBName, "SELECT * from webkitScroll", $single = false, $execute = null);
