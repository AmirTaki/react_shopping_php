        ...
        case "POST" :
            if(
                ...
                if($checkImage){
                    ...
                    $path =  uploadImageMultipart('image', "/back-end/tables/session/webkitScroll/uploadImage/");
                    if($path){

                        // create to database 
                        $ersponse =  operationsDatabase($DBName, "INSERT INTO webkitScroll SET title = ?, image = ?, body = ?, created_at = NOW()", $execute = [$_POST['title'], $path, $_POST['body']]);
