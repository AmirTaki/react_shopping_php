        ...
        case "POST" :
            if(
                ...
                if($checkImage){
                    ...
                    $path =  uploadImageMultipart('image', "/back-end/tables/session/resourceImage/uploadImage/");
                    if($path){

                        // create to database 
                        $ersponse =  operationsDatabase($DBName, "INSERT INTO resourceimage SET title = ?, image = ?, body = ?, created_at = NOW()", $execute = [$_POST['title'], $path, $_POST['body']]);
                ...
                echo json_encode(["message" => "title  & body & image  is requierd !!"]);
                exit();
            }
