        ...
        case "DELETE";
            ...
            if(isset($path[7]) && is_numeric($path[7])){
                ...
                if($read){
                    ...
                    $response = operationsDatabase($DBName, "DELETE FROM resourceimage WHERE id = ?", $execute = [$path[7]]);
            
                    // reading all database
                    $users = readTable ($DBName, "SELECT * from resourceimage", $single = false, $execute = null);
