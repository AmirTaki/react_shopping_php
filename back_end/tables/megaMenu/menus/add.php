    ...
    switch($method){
        case "POST" : 
            ...
            if(isset($user->title) && $user->title !== ""){
                $readAccount = readTable($DBName, 'SELECT * FROM menu WHERE title = ?', true, [$user->title]);  
                
                if(!$readAccount){
                   $response =  operationsDatabase($DBName, "INSERT INTO menu SET Title = ?, created_at = NOW()", $execute = [$user->title]);
