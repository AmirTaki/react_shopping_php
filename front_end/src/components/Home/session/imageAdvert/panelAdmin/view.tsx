...
const ViewImageAdvertSessionPA = () => {
    ...
    useEffect(() => {
        dispatch(onChangeSliderPanelAdmin({id: 11}))
    }, [])
    ...
    return(
        <div className={`flex justify-center items-center flex-col gap-10 ${user?.level !== "A" && user?.level !== 'B' ? "opacity-10": ""}`}>
            <h1 className="text-2xl hover:tracking-[.3rem] duration-300">Image Advert Tables</h1> 
            <Outlet />
        </div>
    )
}
export default ViewImageAdvertSessionPA;