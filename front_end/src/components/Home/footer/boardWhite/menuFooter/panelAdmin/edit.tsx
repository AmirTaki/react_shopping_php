...
import { editMenuFooterThunk, readingMenuFooterThunk } from "../redux/actionsMenuFooter";


const EditMenuFooterPA = () => {
    const {id} = useParams();
   ...
   const {title, addItems} =  useSelector((state: RooState) => state.menuFooter)
    ...
    useEffect(() => {
        ...
        dispatch(readingMenuFooterThunk({id: Number(id)}))
    }, [])
                ...
                <div className="flex justify-center items-center">
                    <input 
                        onClick={(event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
                            ...
                            dispatch(editMenuFooterThunk({id: Number(id), title: title.name}))
                        }}
                        type="submit"
                        value = "update" 
...
export default EditMenuFooterPA;